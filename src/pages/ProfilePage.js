        ...
        <div style={{ maxWidth: "800px", margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "48px" }}>
            <div>
              ...
              <p style={{ margin: 0, color: "var(--text-muted)" }}>Hoş geldin, <span style={{ color: "white", fontWeight: 500 }}>{user.name}</span>
                <span style={{ marginLeft: "8px", padding: "2px 8px", backgroundColor: "rgba(244,63,94,0.2)", color: "var(--primary)", borderRadius: "4px", fontSize: "12px", fontWeight: 600 }}>Admin</span>
              </p>
            </div>
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "48px" }}>
            <div>
              ...
              <h1 style={{ margin: "0 0 8px 0", fontSize: "32px", fontWeight: 700 }}>Profilim</h1>
              <p style={{ margin: 0, color: "var(--text-muted)" }}>Hoş geldin, <span style={{ color: "white", fontWeight: 500 }}>{user.name}</span>
                <span style={{ marginLeft: "8px", padding: "2px 8px", backgroundColor: "rgba(244,63,94,0.2)", color: "var(--primary)", borderRadius: "4px", fontSize: "12px", fontWeight: 600 }}>Organizatör</span>
              </p>
            </div>
            <button onClick={handleLogout} style={{ padding: "10px 20px", backgroundColor: "transparent", color: "#ef4444", border: "1px solid #ef4444", borderRadius: "8px", fontWeight: 600, cursor: "pointer" }}>
              Çıkış Yap
          ...
          <button onClick={() => navigate("/organizer")} style={{
            ...
            <BarChart2 size={20} /> Organizatör Paneline Git
          </button>
        </div>
      </div>
    );
            ...
            user.tickets.map((ticket, index) => (
              <div key={index} style={{ display: "flex", backgroundColor: "white", color: "black", borderRadius: "16px", overflow: "hidden", position: "relative" }}>
                ...
                <div style={{ width: "200px", backgroundColor: "#f8fafc", padding: "24px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                  ...
                  <QRCodeSVG
                    value={`BiletBul|${ticket.refCode}|${ticket.eventName}|${ticket.date}|${ticket.seats.join(",")}`}
                    size={120}
                    bgColor="#ffffff"
                    fgColor="#0f172a"
                    level="M"
